...
async def _verify_transaction_pin(db: AsyncSession, user_id: UUID, transaction_pin: str) -> None:
    """Fetch the user's pin_hash and verify the supplied PIN. Raises ValueError on failure."""

    result = await db.execute(select(User.pin_hash).where(User.id == user_id))
    pin_hash = result.scalar_one_or_none()
    if pin_hash is None:
        raise ValueError("Transaction PIN not set. Please set a PIN in your profile before making payments.")
    if not verify_pin(transaction_pin, pin_hash):
...
async def withdraw(db: AsyncSession, user_id: UUID, amount: Decimal, idempotency_key: str, transaction_pin: str = "") -> Transaction:
...
async def transfer_p2p(
    ...
    await _verify_transaction_pin(db, sender_user_id, transaction_pin)
...
async def pay_qr(
    ...
    return await pay_merchant(db, sender_user_id, merchant_id, amount, idempotency_key, transaction_pin=transaction_pin)
