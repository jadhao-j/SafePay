...
async def add_money(
    ...
    try:
        txn = await wallet_service.add_money(db, user_id, payload.amount, payload.idempotency_key, payload.transaction_pin)
    except ValueError as exc:
...
async def withdraw(
    ...
    try:
        txn = await wallet_service.withdraw(db, user_id, payload.amount, payload.idempotency_key, payload.transaction_pin)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)) from exc
