...
export default function WalletPage(): JSX.Element {
  ...
  function handlePinKey(val: string): void {
    ...
    } else if (modalPin.length < 6) {
      const next = modalPin + val;
      setModalPin(next);
      if (next.length === 6) setTimeout(() => void handleAction(next), 200);
    }
  }
  ...
  async function handleAction(pinVal?: string): Promise<void> {
    ...
    try {
      ...
      // Add money bypasses PIN; withdraw requires it
      if (modal === "withdraw") {
        payload.transaction_pin = pinVal ?? modalPin;
      } else {
        payload.transaction_pin = "000000"; // placeholder — backend ignores for add-money
      }
      await apiClient.post(modal === "add" ? "/wallet/add-money" : "/wallet/withdraw", payload);
        ...
        {!loading && filteredTxns.map(txn => {
          ...
          return (
            <Link key={txn.id} href={`/history/${txn.id}`} id={`txn-wallet-${txn.id}`}
              ...
              <div style={{
                width: 38, height: 38, borderRadius: 11, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15,
            ...
            {modalStep === "amount" && (
              <>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
                  {["500", "1000", "2000", "5000"].map(q => (
                    <button key={q} onClick={() => setModalAmt(q)}
                      style={{
                        flex: 1, padding: "8px 4px", borderRadius: 999, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "var(--font-ibm-plex-mono,monospace)",
                ...
                <button id={`btn-wallet-${modal}-next`}
                  onClick={() => { if (parseFloat(modalAmt) > 0) { if (modal === "add") { void handleAction("000000"); } else { setModalStep("pin"); setModalPin(""); setModalError(null); } } }}
            ...
            {modalStep === "pin" && (
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                ...
                <div style={{ display: "flex", gap: 14, margin: "18px 0 22px" }}>
                  {[0, 1, 2, 3, 4, 5].map(i => <PinDot key={i} filled={i < modalPin.length} />)}
                </div>
                {/* Numpad */}
